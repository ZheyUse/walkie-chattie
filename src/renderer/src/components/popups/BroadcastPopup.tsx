...
function getBroadcastFontSize(message: string) {
  const len = message.length
  if (len <= 10) return "8vw"
  if (len <= 25) return "6vw"
  if (len <= 50) return "4.5vw"
  if (len <= 90) return "3.5vw"
  if (len <= 140) return "2.8vw"
  return "2.2vw"
}
...
export default function BroadcastPopup() {
  ...
  useEffect(() => {
    function handleClick() { window.api.closePopup() }
      ...
      >
        ...
        >
          ...
          <div className="ml-auto flex items-center gap-2.5">
            <div
              ...
              style={{ background: 'rgba(34,197,94,0.9)', boxShadow: '0 0 8px rgba(34,197,94,0.8)' }}
            />
            <span className="font-display text-sm uppercase tracking-widest" style={{ color: 'rgba(34,197,94,0.8)' }}>
              @all
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-16 overflow-hidden">
          <p
            ...
            style={{
              fontSize: getBroadcastFontSize(data.message),
          ...
          {data.gifUrl && (
            <div className="mt-10 rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(34,197,94,0.2)' }}>
        ...
        >
          <span className="font-body text-sm" style={{ color: 'rgba(34,197,94,0.4)' }}>
            Auto-closes in {countdown}s · Click anywhere to dismiss
          </span>
          <span className="font-display text-base font-medium" style={{ color: 'rgba(34,197,94,0.7)' }}>
            — {data.sender}
          </span>
        </div>
      </div>
