...
function getPopupData(): PopupData {
  try {
    ...
    if (parts[1] === "popup" && parts[2]) {
      ...
      debugLog({ source: "shout-popup", message: "[SUCCESS] PopupData parsed", details: { hasImageUrl: !!data.imageUrl, imageUrl: data.imageUrl, hasGifUrl: !!data.gifUrl, gifUrl: data.gifUrl } })
      return data
    }
  } catch(e) {
    debugLog({ level: "error", source: "shout-popup", message: "[ERROR] Failed to parse PopupData", details: e })
  }
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-16 overflow-hidden">
          ...
          {data.imageUrl && (
            <div className="mt-10 rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(232,101,42,0.2)' }}>
              {imageLoading && (
                <div className="w-full h-64 flex items-center justify-center" style={{ background: 'rgba(232,101,42,0.05)' }}>
                  <div className="w-10 h-10 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: 'rgba(232,101,42,0.3)', borderTopColor: 'transparent' }} />
                </div>
              )}
